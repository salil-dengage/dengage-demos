(function () {
    ...
    function renderBreadcrumb(product) {
        ...
        document.title = 'NovaPay | ' + product.name;
    }
    ...
    function renderProduct(product) {
        ...
        const price = window.NovaPayCatalogData.formatPrice(product.price, product.currency);
        const oldPrice = product.oldPrice
            ? '<span class="product-detail-old-price">' + escapeHtml(window.NovaPayCatalogData.formatPrice(product.oldPrice, product.currency)) + '</span>'
            : '';
        ...
        const colorsHtml = product.colors && product.colors.length
            ? '<div class="product-detail-meta-row">'
              + '<span class="product-detail-meta-label">Key terms</span>'
        ...
        const brandHtml = product.brand
            ? '<div class="product-detail-meta-row">'
              + '<span class="product-detail-meta-label">Provider</span>'
        ...
        const availabilityHtml = '<div class="product-detail-meta-row">'
            + '<span class="product-detail-meta-label">Status</span>'
            + '<span class="product-detail-meta-value ' + (product.availability ? 'is-available' : 'is-unavailable') + '">'
            + (product.availability ? 'Open to apply' : 'Waitlist')
            + '</span>'
            + '</div>';
        ...
        container.innerHTML = ''
            ...
            + '>'
            + (product.availability ? 'Add to application' : 'Waitlist')
            + '      </button>'
            + '    </div>'
    ...
    function bindActions(container, product) {
        ...
        if (addBtn) {
            addBtn.addEventListener('click', function () {
                if (addBtn.disabled || !window.NovaPayCart) return;
                ...
                window.NovaPayCart.addItem({
                    ...
                    image: product.image,
                    category: getCategoryParts(product.category).pop() || 'Products'
                }, quantity);
            });
        }
    }
    ...
    function renderError(message) {
        ...
        container.innerHTML = ''
            ...
            + '  <a class="btn-primary" href="index.html#products">Back to products</a>'
            + '</div>';
    }
    ...
    function init() {
        ...
        if (!window.NovaPayCatalogData) {
            renderError('Catalog is not available.');
            return;
        }

        window.NovaPayCatalogData.getProductById(productId)
            .then(function (product) {
                ...
                window.dispatchEvent(new CustomEvent('novapay:product:loaded', { detail: { product: product } }));
            })
