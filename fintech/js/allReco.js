/* ============================================================================
   Recommendation widget source

   By default the five widgets read the local catalogue
   (novapay_products.json), so the demo works before anything is configured in
   the panel.

   Fill in a containerKey below and that widget switches to Dengage's own
   recommendation engine:

     dengage('getRecommendation', containerKey, opts, callback)

   Left in Portuguese from the CantuPneus fork until 31 July 2026. Translated
   because a comment nobody can read is a comment nobody maintains, and this
   file is the one a prospect's developer is most likely to be shown.
============================================================================ */
var NOVAPAY_RECO_CONTAINERS = {
  classicWidget:    '',
  bannerWidget:     '',
  tabWidget:        '',
  mostViewedWidget: '',
...
function novapayMapRecommendation(list) {
...
function novapayLoadProducts(widgetId, productsUrl, onReady, label) {
  var containerKey = NOVAPAY_RECO_CONTAINERS[widgetId];
  ...
  function finish(list) {
    if (settled) return;
    settled = true;
    var mapped = novapayMapRecommendation(list);
...
function ClassicWidget() {
    const PRODUCTS_URL = 'novapay_products.json';
    ...
    function loadProducts(onReady) {
        novapayLoadProducts(WIDGET_ID, PRODUCTS_URL, onReady, 'ClassicWidget');
    }
    ...
    function bindCartButtons(root) {
        if (!window.NovaPayCart || !root) return;
        root.querySelectorAll('[data-cart-add]:not([data-reco-bound])').forEach((btn) => {
            btn.dataset.recoBound = 'true';
            btn.addEventListener('click', (e) => {
                e.preventDefault();
                window.NovaPayCart.addItem({
                    id: btn.dataset.productId,
                    name: btn.dataset.productName,
                    price: btn.dataset.productPrice,
                    currency: btn.dataset.productCurrency || 'USD',
                    image: btn.dataset.productImage,
                    category: btn.dataset.productCategory
                });
                window.dispatchEvent(new CustomEvent('novapay:cart:updated'));
                const toast = document.getElementById('loginSuccessMessage');
                if (toast) {
                    toast.textContent = 'Added to your application.';
                    toast.classList.add('active');
                    clearTimeout(window.novapayToastTimer);
                    window.novapayToastTimer = setTimeout(() => toast.classList.remove('active'), 2000);
                }
            });
        });
    }
            ...
            <div class="container">
                <div class="reco-widget-header">
                    <div>
                        ...
                        <h2 class="reco-widget-title">Most opened</h2>
                        <p class="reco-widget-subtitle">The products our customers open most this quarter</p>
                    </div>
                </div>
...
function BannerWidget() {
    ...
    function loadProducts(onReady) {
        novapayLoadProducts(WIDGET_ID, PRODUCTS_URL, onReady, 'BannerWidget');
    }
            ...
            <div class="container">
                <div class="banner-widget-header">
                    <div class="banner-widget-heading">
                        <p class="text-label">Product team pick</p>
                        <h2 class="banner-widget-title">Recommended for you</h2>
                        <p class="banner-widget-subtitle">Products matched to how you spend and save</p>
                    </div>
                ...
                <div class="banner-widget-body">
                    <aside class="banner-widget-banner">
                        ...
                        <div class="banner-widget-banner-overlay">
                            ...
                            <a href="#collections" class="banner-widget-banner-link">See all products</a>
                        </div>
                    </aside>
...
function TabWidget() {
    ...
    /* match is tested against the feed's category path, for example
       "Products > Cards > Premium". Keep these in step with
       novapay_products.json or a tab renders empty. */
    const CATEGORIES = [
        { id: 'cards', label: 'Cards', match: '> Cards >' },
        { id: 'investing', label: 'Investing', match: '> Investing >' },
        { id: 'savings', label: 'Savings', match: '> Savings >' },
        { id: 'protection', label: 'Protection', match: '> Protection >' }
    ];
    ...
    function loadProducts(onReady) {
        novapayLoadProducts(WIDGET_ID, PRODUCTS_URL, onReady, 'TabWidget');
    }
            ...
            <div class="container">
                <div class="tab-widget-header">
                    <div class="tab-widget-heading">
                        ...
                        <h2 class="tab-widget-title">Most opened in this category</h2>
                        <p class="tab-widget-subtitle">The products most often added alongside this one</p>
                    </div>
                </div>
...
function SideBarWidget() {
    ...
    function loadProducts(onReady) {
        novapayLoadProducts(WIDGET_ID, PRODUCTS_URL, onReady, 'MostViewedWidget');
    }
...
function PopupWidget() {
    ...
    function loadProducts(onReady) {
        novapayLoadProducts(WIDGET_ID, PRODUCTS_URL, onReady, 'PopupWidget');
    }
