...
const USER_STORAGE_KEY = 'novapay_user';
...
/* ------------------------------------------------------- bounced from the portal
   js/novapayGate.js sends anyone without a session back here with ?signin=1.
   Reopen the form and say why, otherwise the redirect reads as the portal
   link being broken.

   The dataLayer push is the interesting half: someone who tried to reach the
   portal and could not is a real intent signal, and it is the kind of thing a
   prospect asks whether they can act on. They can, with an On-Site campaign on
   fintech_portal_gate_blocked. */
if (/[?&]signin=1/.test(window.location.search)) {
   const gateNote = document.querySelector('.login-modal-text');
   if (gateNote) {
      gateNote.textContent =
         'Your portal is for account holders. Sign in and we will take you straight there.';
   }

   window.dataLayer = window.dataLayer || [];
   window.dataLayer.push({ event: 'fintech_portal_gate_blocked' });

   /* After paint, so the modal animates in rather than appearing already open. */
...
if (loginForm) {
   loginForm.addEventListener('submit', event => {
      ...
      const userId = 'NPY-' + Math.floor(10000 + Math.random() * 90000);
      ...
      /* Into the portal. novapay_user is written above, so js/novapayGate.js
         lets them through on arrival; this is the only door.

         The short beat before navigating is not decoration. setContactKey and
         the dataLayer push above have just gone out, and the SDK is
         asynchronous: navigating in the same tick can cut off the identify
         call, which is exactly how events end up on the anonymous device
         profile instead of the contact. It also gives the welcome toast time
         to register, so the transition reads as a sign-in rather than a jump.

         ?ck= is carried across, because that is how any contact is demoed. */
      var ck = /[?&]ck=([^&#]+)/.exec(window.location.search);
      setTimeout(function () {
         window.location.href = 'app.html' + (ck ? '?ck=' + ck[1] : '');
      }, 900);
   });
}
