package com.dengagefintech.demo.inapp

/* ============================================================================
   THE NOVAPAY IN-APP CATALOGUE
   ...
   THE SCREEN NAMES ARE NOT APP PREFIXED, AND THAT IS DELIBERATE. Meridian
   Bank reports the same test_inapp_* vocabulary. The two never collide,
   because an In-App campaign selects its Applications, and NovaPay and
   Meridian are separate Android applications in the panel. The upside is that
   one set of words describes both finance demos. The one thing to watch is
   the Applications field: leaving both apps ticked puts a NovaPay message in
   front of a Meridian audience.
   ...
   AND ONE THAT LOOKS LIKE A FAULT AND IS NOT. Delivery Control caps how often
   a visitor sees a campaign, and the default "once in every N minutes" window
   is per visitor and per campaign. It survives a force stop and a reinstall
   is not a reset either, because the cap is held against the visitor rather
   than the process. So a message that showed once and then stops on repeated
   relaunches is usually the cap doing its job. Widen it in the panel while
   rehearsing, and set it back before the call.
   ========================================================================== */
object NovaPayInApp {
     ...
     * @param file     the content to paste, in fintech/panel-content/mobile/.
     */
    ...
    val PLACEMENTS: List<Placement> = listOf(
        ...
        Placement(
            ...
            "A wheel, in a money app's register. CSS only: the panel strips script tags."
        ),
