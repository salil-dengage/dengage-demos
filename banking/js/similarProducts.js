(function () {
    ...
    function buildProductCard(product) {
        ...
        const oldPrice = Number(product.oldPrice) > Number(product.price)
            ...
            : '';
        const detailUrl = window.MeridianCatalogData
            ? window.MeridianCatalogData.buildProductUrl(product.id)
        ...
        return ''
            ...
            + '>' + (product.availability ? 'Add to application' : 'Register interest')
            + '</button>'
            + '    </div>'
            + '  </article>'
            + '</div>';
    ...
    function render(currentProduct) {
        if (!window.MeridianCatalogData) return;
        ...
        window.MeridianCatalogData.loadProducts()
            .then(function (allProducts) {
                ...
                const subtitle = categoryLabel
                    ...
                    : 'More from the same product family';
    ...
    window.addEventListener('meridian:product:loaded', function (event) {
