/* ============================================================================
   Dengage On-Site scenario catalog  |  Meridian Bank (Banking demo)
   ----------------------------------------------------------------------------
   TWO EVENT CONTRACTS:

   1) Default Scenarios  -> banking_<slug>, PREFIXED since 2026-07-31
      { event: 'banking_<slug>', actionType: '<slug>', category: 'Default Scenarios' }
      ...
      equal to banking_<slug>.

      They used to fire the bare slug and share the unprefixed campaigns with
      FinTech. One campaign holds one piece of content, so that meant a UK
      bank showing tyre-shop creative whose CTA landed on a tyre shop. The
      replacement content is in banking/panel-content/.

      The slug spelling is unchanged, misspellings included: those are the
      panel contract. A scenario with no matching campaign is silently dark.

   2) Every other group  -> a banking-prefixed event name
      { event: 'banking_<slug>', actionType: '<slug>', widgetName: '<name>',
        category: '<group>' }
      ...
      their own namespace, next to fintech_ and the CantuPneus events.
   ========================================================================== */

/* ----------------------------------------------------------------------------
   Big Data table that receives catalog events.

   Create a Big Data table in the panel with this name and these columns,
   then trigger On-Site scenarios on event_name. key and event_date are
   filled by the SDK.

     event_name      text   scenario slug, e.g. survey, nps-popup
     scenario_group  text   catalog group, e.g. Default Scenarios
     widget_name     text   widget name, e.g. Nps Popup
     page_type       text   home | product
     page_url        text   page URL at click time

   Renamed the table in the panel? Change this constant only.
---------------------------------------------------------------------------- */
var DENGAGE_EVENT_TABLE = 'banking_onsite_events';

/* No scenario-event prefix on this site, deliberately.

   The two CantuPneus sites prefix theirs with br_ and en_ so each can carry its
   own language of content in the panel. This site keeps the bare slugs, so the
   original unprefixed campaigns still serve it. Delete those campaigns and this
   site's eight Default Scenarios go dark.

   If this site ever needs its own content, give it a prefix the same way
   cantuCatalog.js does and build the matching campaigns. */

/* Prefix for this site's own dataLayer events. The 8 Default Scenarios are
   exempt on purpose, see the note above. */
var EVENT_PREFIX = 'banking_';
...
function meridianCatalog() {
  ...
  var menuData = [
    ...
    },
    {
      ...
      items: [
        { label: 'Mega Banner',          widgetName: 'Mega Banner',       slug: 'mega-banner',       reveal: '#meridian-highlights-slider', fn: function () { window.showSliderBanner(); } },
    ...
    },
    {
      ...
      icon: '◆',
      note: 'Powered by the Meridian product catalog',
      items: [
  ...
  menuData.forEach(function (menu) {
    ...
    menu.items.forEach(function (item) {
      ...
      label.innerHTML =
        ...
        (menu.mode === 'panel' ? 'event: ' + EVENT_PREFIX : '') + item.slug + '</small>';
      ...
      btn.addEventListener('click', function (e) {
        ...
        if (menu.mode === 'panel') {
          /* Default Scenarios now fire PREFIXED, like every other event on
             this site. Flipped on Salil's instruction once the eight
             banking_ campaigns were ready to be built.

             The slug spelling is untouched, including the three misspellings
             that are part of the panel contract, so the triggers are
             banking_survey, banking_nps-popup, banking_subscripton-popup,
             banking_stickey-bar, banking_image-popup, banking_image-bar,
             banking_horizonal-popup and banking_cta-image-popup.

             A scenario appears ONLY if a campaign exists with that exact
             trigger name. Any of the eight without one is silently dark:
             nothing errors, the widget simply never shows. */
