/* Dengage Event Modal Controller, Meridian Bank
 * Event panel for the banking demo: one card per banking_* table
 */
(function () {
    'use strict';
    ...
    const LANGUAGES = {
        pt: {
            ...
            modalSubtitle: 'Edit the payloads and fire Meridian events straight into Dengage.',
            helpTitle: '// eventos nativos do SDK -> tabelas padrão do Data Space',
            helpText: 'The first card fires the native <em>pageView</em>, which fills page_view_events and is what the On-Site scenarios trigger on. Every other card is a <em>sendDeviceEvent</em> into one of the nine purpose-built banking_* tables. There are no <em>ec:*</em> calls on this site: a mortgage is not a basket item. <em>key</em>, <em>event_date</em> and <em>session_id</em> are filled by the SDK.',
            ...
            events: [
                ...
                { title: 'Product shortlisted', description: 'sendDeviceEvent → banking_product_events. The rate-drop trigger.' },
                { title: 'Mortgage affordability run', description: 'sendDeviceEvent → banking_tool_events, with the figures the customer typed.' },
                { title: 'Application abandoned mid-step', description: 'sendDeviceEvent → banking_application_events, naming the step.' },
                { title: 'Appointment booked', description: 'sendDeviceEvent → banking_appointment_events, with lead time.' },
                { title: 'Overdraft entered', description: 'sendDeviceEvent → banking_account_events.' },
                { title: 'Foreign transaction', description: 'sendDeviceEvent → banking_transaction_events. Real-time travel trigger.' },
                { title: 'Travel notice set', description: 'sendDeviceEvent → banking_card_events, with country and dates.' },
                { title: 'Portfolio viewed', description: 'sendDeviceEvent → banking_wealth_events, value banded not exact.' },
                { title: 'Marketing consent updated', description: 'sendDeviceEvent → banking_engagement_events.' }
            ]
        },
        en: {
    ...
    const EVENT_TEMPLATES = [
        {
            /* Product viewed -> page_view_events. The one first-class SDK call
               this site still makes. No price and no stock_count: a mortgage
               has neither, and an invented figure poisons every segment. */
            action: 'pageView',
            tableName: 'page_view_events',
            properties: [
                { key: 'page_type', value: 'product' },
                { key: 'page_url', value: 'https://salil-dengage.github.io/dengage-demos/banking/product.html?id=MRD-MTG-FIX5' },
                { key: 'page_title', value: 'Five Year Fixed Rate Mortgage | Meridian Bank' },
                { key: 'product_id', value: 'MRD-MTG-FIX5' },
                { key: 'category_path', value: 'Products > Mortgages > Fixed rate' }
            ]
        },
        {
            /* Shortlisted -> banking_product_events. Pair this with a change to
               headline_rate in banking_products and you have the rate-drop
               campaign in one row. */
            tableName: 'banking_product_events',
            properties: [
                { key: 'event_type', value: 'product_shortlisted' },
                { key: 'event_source', value: 'web' },
                { key: 'customer_tier', value: 'prospect' },
                { key: 'is_authenticated', value: 'false' },
                { key: 'product_id', value: 'MRD-MTG-FIX5' },
                { key: 'product_name', value: 'Five Year Fixed Rate Mortgage' },
                { key: 'product_category', value: 'mortgage' },
                { key: 'headline_rate', value: '4.09' },
                { key: 'rate_type', value: 'fixed' },
                { key: 'term_months', value: '60' },
                { key: 'fee_amount', value: '1499' },
                { key: 'min_deposit_pct', value: '25' }
            ]
        },
        {
            /* banking_tool_events. The richest row on the public site, because
               the customer volunteered every input. result_max_borrow below
               input_amount is its own segment: they want more than they can have. */
            tableName: 'banking_tool_events',
            properties: [
                { key: 'event_type', value: 'mortgage_affordability_calculated' },
                { key: 'event_source', value: 'web' },
                { key: 'customer_tier', value: 'prospect' },
                { key: 'tool_name', value: 'mortgage_affordability' },
                { key: 'product_category', value: 'mortgage' },
                { key: 'input_amount', value: '420000' },
                { key: 'input_deposit', value: '42000' },
                { key: 'input_income_annual', value: '78000' },
                { key: 'input_outgoings_monthly', value: '1450' },
                { key: 'result_max_borrow', value: '351000' },
                { key: 'loan_to_value_pct', value: '90' },
                { key: 'completed', value: 'true' }
            ]
        },
        {
            /* banking_application_events. Step-level, which is the entire
               reason this site does not use the ecommerce funnel. */
            tableName: 'banking_application_events',
            properties: [
                { key: 'event_type', value: 'step_abandoned' },
                { key: 'event_source', value: 'web' },
                { key: 'customer_tier', value: 'prospect' },
                { key: 'application_id', value: 'APP-' + Date.now() + '' },
                { key: 'product_id', value: 'MRD-MTG-FIX5' },
                { key: 'product_category', value: 'mortgage' },
                { key: 'step_name', value: 'identity_verification' },
                { key: 'step_index', value: '4' },
                { key: 'total_steps', value: '6' },
                { key: 'time_on_step_seconds', value: '184' },
                { key: 'abandoned_at_step', value: 'identity_verification' },
                { key: 'documents_outstanding', value: '2' }
            ]
        },
        {
            tableName: 'banking_appointment_events',
            properties: [
                { key: 'event_type', value: 'appointment_booked' },
                { key: 'event_source', value: 'web' },
                { key: 'customer_tier', value: 'classic' },
                { key: 'appointment_id', value: 'APT-' + Date.now() + '' },
                { key: 'appointment_type', value: 'mortgage_advice' },
                { key: 'appointment_channel', value: 'video' },
                { key: 'branch_name', value: 'Leeds City' },
                { key: 'branch_city', value: 'Leeds' },
                { key: 'adviser_name', value: 'J. Okafor' },
                { key: 'scheduled_at', value: '2026-08-14T10:30:00Z' },
                { key: 'lead_time_hours', value: '72' },
                { key: 'product_category', value: 'mortgage' }
            ]
        },
        {
            /* banking_account_events. Three months of this in a row is a
               consolidation loan conversation, not a marketing email. */
            tableName: 'banking_account_events',
            properties: [
                { key: 'event_type', value: 'overdraft_entered' },
                { key: 'event_source', value: 'web' },
                { key: 'customer_tier', value: 'classic' },
                { key: 'is_authenticated', value: 'true' },
                { key: 'account_id_masked', value: '****4471' },
                { key: 'account_type', value: 'current_account' },
                { key: 'balance_amount', value: '-182.40' },
                { key: 'balance_band', value: 'overdrawn' },
                { key: 'currency', value: 'GBP' },
                { key: 'overdraft_limit', value: '500.00' },
                { key: 'overdraft_used', value: '182.40' }
            ]
        },
        {
            /* banking_transaction_events. The strongest real-time trigger on
               the site: a known country, right now. */
            tableName: 'banking_transaction_events',
            properties: [
                { key: 'event_type', value: 'foreign_transaction' },
                { key: 'event_source', value: 'web' },
                { key: 'customer_tier', value: 'premier' },
                { key: 'is_authenticated', value: 'true' },
                { key: 'transaction_id', value: 'TXN-' + Date.now() + '' },
                { key: 'account_id_masked', value: '****4471' },
                { key: 'amount', value: '84.20' },
                { key: 'currency', value: 'GBP' },
                { key: 'direction', value: 'debit' },
                { key: 'merchant_name', value: 'SNCF Connect' },
                { key: 'merchant_category', value: 'travel' },
                { key: 'mcc', value: '4112' },
                { key: 'country_code', value: 'FR' },
                { key: 'is_foreign', value: 'true' },
                { key: 'payment_channel', value: 'mobile_wallet' }
            ]
        },
        {
            /* banking_card_events. A known destination AND a known date range,
               so travel cover and FX are both timely rather than guessed. */
            tableName: 'banking_card_events',
            properties: [
                { key: 'event_type', value: 'travel_notice_set' },
                { key: 'event_source', value: 'web' },
                { key: 'customer_tier', value: 'premier' },
                { key: 'is_authenticated', value: 'true' },
                { key: 'card_id_masked', value: '****8820' },
                { key: 'card_type', value: 'credit' },
                { key: 'card_product', value: 'Meridian Platinum Card' },
                { key: 'travel_country', value: 'ES' },
                { key: 'travel_start_date', value: '2026-08-20T00:00:00Z' },
                { key: 'travel_end_date', value: '2026-09-03T00:00:00Z' }
            ]
        },
        {
            /* banking_wealth_events. Value is banded, never exact: a demo that
               shows a named customer's precise portfolio value reads badly. */
            tableName: 'banking_wealth_events',
            properties: [
                { key: 'event_type', value: 'portfolio_viewed' },
                { key: 'event_source', value: 'web' },
                { key: 'customer_tier', value: 'private' },
                { key: 'is_authenticated', value: 'true' },
                { key: 'portfolio_id', value: 'PF-3391' },
                { key: 'portfolio_value_band', value: '250k_500k' },
                { key: 'risk_profile', value: 'balanced' },
                { key: 'adviser_name', value: 'R. Mehta' },
                { key: 'performance_band', value: 'up_5_10' }
            ]
        },
        {
            /* banking_engagement_events. Consent is modelled explicitly because
               a UK bank cannot demo personalisation without being asked about
               it. Showing a preference change land answers it in advance. */
            tableName: 'banking_engagement_events',
            properties: [
                { key: 'event_type', value: 'preference_updated' },
                { key: 'event_source', value: 'web' },
                { key: 'customer_tier', value: 'classic' },
                { key: 'is_authenticated', value: 'true' },
                { key: 'consent_email', value: 'true' },
                { key: 'consent_sms', value: 'false' },
                { key: 'consent_push', value: 'true' },
                { key: 'consent_profiling', value: 'true' }
            ]
        }
    ];
