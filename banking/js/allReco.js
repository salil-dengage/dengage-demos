/* ============================================================================
   ...
   Por padrão os cinco widgets leem o catálogo local (meridian_products.json),
...
function meridianMapRecommendation(list) {
  return (list || []).map(function (p) {
    return {
      ...
      link: p.link || '',
      category: p.categoryPath || p.category || '',
      brand: p.brand || '',
...
function meridianLoadProducts(widgetId, productsUrl, onReady, label) {
  var containerKey = MERIDIAN_RECO_CONTAINERS[widgetId];
  ...
  function local() {
    fetch(productsUrl)
      ...
      /* Normalise the raw feed the same way the engine path does. The feed
         carries a typed category ("mortgage") AND a display path ("Products >
         Mortgages > Fixed rate"); the widgets match on the path. Passing the
         raw rows through meant every tab matched nothing and the tab widget
         rendered empty with no error at all. */
      .then(function (data) {
        var list = Array.isArray(data) ? data : [];
        onReady(list.map(function (p) {
  ...
  function finish(list) {
    if (settled) return;
    settled = true;
    var mapped = meridianMapRecommendation(list);
...
function ClassicWidget() {
    const PRODUCTS_URL = 'meridian_products.json';
    const WIDGET_ID = 'classicWidget';
    const SWIPER_CLASS = 'classicWidgetSwiper';

    function getCollectionsSection() {
        /* The homepage no longer has a .collections section: that was the
           tyre shop's "product lines" block. Recommendations now sit above the
           calculators, which is where a bank would put them. The old selector
           is kept as a fallback so this cannot silently render nothing, which
    ...
    function mountWidget(widgetEl) {
        ...
        if (!collections) {
            console.warn('ClassicWidget: no .tools or .collections anchor found');
            return false;
        }
    ...
    function loadProducts(onReady) {
        meridianLoadProducts(WIDGET_ID, PRODUCTS_URL, onReady, 'ClassicWidget');
    }
                ...
                <article class="reco-product-card">
                    ...
                    <div class="reco-product-body">
                        ...
                        <button class="reco-product-cart-btn" type="button" data-cart-add
                            ...
                            data-product-currency="${escapeHtml(product.currency || 'GBP')}"
                            data-product-image="${escapeHtml(image)}"
                            data-product-category="${escapeHtml(product.category)}"
                            ${product.availability ? '' : 'disabled'}
                        >${product.availability ? 'Add to application' : 'Register interest'}</button>
                    </div>
                </article>
            </div>
        `;
    ...
    function bindCartButtons(root) {
        if (!window.MeridianCart || !root) return;
        root.querySelectorAll('[data-cart-add]:not([data-reco-bound])').forEach((btn) => {
            btn.dataset.recoBound = 'true';
            btn.addEventListener('click', (e) => {
                e.preventDefault();
                window.MeridianCart.addItem({
                    id: btn.dataset.productId,
                    name: btn.dataset.productName,
                    price: btn.dataset.productPrice,
                    currency: btn.dataset.productCurrency || 'GBP',
                    image: btn.dataset.productImage,
                    category: btn.dataset.productCategory
                });
                window.dispatchEvent(new CustomEvent('meridian:cart:updated'));
                const toast = document.getElementById('loginSuccessMessage');
                if (toast) {
                    toast.textContent = 'Added to cart.';
                    toast.classList.add('active');
                    clearTimeout(window.dMeridianToastTimer);
                    window.dMeridianToastTimer = setTimeout(() => toast.classList.remove('active'), 2000);
                }
            });
        });
    }
            ...
            <div class="container">
                <div class="reco-widget-header">
                    <div>
                        ...
                        <h2 class="reco-widget-title">Most applied for</h2>
                        <p class="reco-widget-subtitle">The products our customers apply for most this quarter</p>
                    </div>
                </div>
...
function BannerWidget() {
    ...
    function mountWidget(widgetEl) {
        ...
        const collections = document.querySelector('.tools')
            || document.querySelector('.collections');
        if (!collections) {
    ...
    function loadProducts(onReady) {
        meridianLoadProducts(WIDGET_ID, PRODUCTS_URL, onReady, 'BannerWidget');
    }
            ...
            <div class="container">
                <div class="banner-widget-header">
                    <div class="banner-widget-heading">
                        <p class="text-label">Adviser's pick</p>
                        <h2 class="banner-widget-title">Recommended for you</h2>
                        <p class="banner-widget-subtitle">Products matched to your banking profile</p>
                    </div>
                ...
                <div class="banner-widget-body">
                    <aside class="banner-widget-banner">
                        ...
                        <div class="banner-widget-banner-overlay">
                            ...
                            <a href="#products" class="banner-widget-banner-link">See all products</a>
                        </div>
                    </aside>
...
function TabWidget() {
    ...
    /* match is tested against the feed's category path, for example
       "Products > Savings > ISA". Keep these in step with
       meridian_products.json or a tab renders empty. */
    const CATEGORIES = [
        { id: 'accounts', label: 'Current accounts', match: '> Current accounts >' },
        { id: 'cards', label: 'Credit cards', match: '> Credit cards >' },
        { id: 'mortgages', label: 'Mortgages', match: '> Mortgages >' },
        { id: 'savings', label: 'Savings', match: '> Savings >' }
    ];
    ...
    function loadProducts(onReady) {
        meridianLoadProducts(WIDGET_ID, PRODUCTS_URL, onReady, 'TabWidget');
    }
            ...
            <div class="container">
                <div class="tab-widget-header">
                    <div class="tab-widget-heading">
                        ...
                        <h2 class="tab-widget-title">Most applied for in this category</h2>
                        <p class="tab-widget-subtitle">The products most often held alongside this one</p>
                    </div>
                </div>
...
function SideBarWidget() {
    ...
    function loadProducts(onReady) {
        meridianLoadProducts(WIDGET_ID, PRODUCTS_URL, onReady, 'MostViewedWidget');
    }
...
function PopupWidget() {
    ...
    function loadProducts(onReady) {
        meridianLoadProducts(WIDGET_ID, PRODUCTS_URL, onReady, 'PopupWidget');
    }
        ...
        overlay.innerHTML = `
            <div id="${WIDGET_ID}" role="dialog" aria-modal="true" aria-labelledby="${WIDGET_ID}-title">
                ...
                <div class="popup-header">
                    ...
                    <p class="popup-subtitle">Chosen from the products you have viewed</p>
                </div>
