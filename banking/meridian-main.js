...
const USER_STORAGE_KEY = 'meridian_user';
...
if (loginForm) {
   loginForm.addEventListener('submit', event => {
      ...
      const userId = 'MRD-' + Math.floor(10000 + Math.random() * 90000);
      ...
      /* Registering lands you in online banking, which is what a bank does and
         what makes the signed-in half of the demo reachable without hunting
         for a URL. Delayed so the welcome toast is actually seen. */
      setTimeout(function () { window.location.href = 'dashboard.html'; }, 1200);
   });
}

/* A signed-in visitor gets the portal from the person icon instead of the
   registration modal. No new header control: there is no horizontal slack in
   that nav, which is documented and has bitten before. */
(function () {
   let signedIn = false;
   try { signedIn = !!(JSON.parse(localStorage.getItem(USER_STORAGE_KEY) || 'null') || {}).email; }
   catch (err) { signedIn = false; }
   if (!signedIn) return;

   document.querySelectorAll('[data-login-btn]').forEach(btn => {
      btn.setAttribute('aria-label', 'Go to online banking');
      if (btn.classList.contains('mobile-login-btn')) btn.textContent = 'Online banking';
...
function changeSlide() {
   /* Only the home page has a hero. Every other page loads this file for the
      nav, the modal and the smooth scrolling, and the interval below used to
      run there too, throwing a TypeError every four seconds against an empty
      slide list. Nothing broke visibly, which is why it survived: the console
      error was the only symptom. */
   if (!slides.length || !heroTitle || !heroPrice) return;
