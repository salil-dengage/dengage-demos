...
function addIcon() {
    if (!document.querySelector("#dng-notification-container")) {
      ...
      const items = [
        {
          title: "Неделя дальнобойщика: превью",
          description: "Свяжитесь с консультантом и соберите полную загрузку до старта акции.",
          url: "#contact",
          icon: "gift",
          type: "buttons", // 2 butonlu tip
          buttons: [
            { text: "Связаться с консультантом", url: "#contact", primary: true },
            { text: "Смотреть линейки", url: "#collections", primary: false }
          ],
        ...
        },
        {
          ...
          description: "Новая партия Marshal KLD01 295/80 R22.5 доступна ограниченное время.",
          url: "#collections",
          icon: "card",
        ...
        {
          title: "Бесплатная доставка при полной загрузке",
          description: "Рассчитайте полную загрузку и посмотрите, как доставка CIF применяется к заказу.",
          url: "#contact",
          icon: "gift",
          type: "buttons", // 2 butonlu tip
          buttons: [
            { text: "Book Care", url: "#contact", primary: true },
            { text: "Подробнее", url: "#craftsmanship", primary: false }
          ],
          startDate: "Jan 01 2025 00:00",
          endDate: "Dec 31 2026 23:59",
        },
        {
          title: "Расчёт полной загрузки",
          description: "Соберите заказ по сельхозлинейке с помощью консультанта, от расчёта до доставки в хозяйство.",
          url: "#contact",
          icon: "campus",
          type: "countdown", // Sayaçlı tip
        ...
        {
          title: "Смотреть сельскохозяйственную линейку",
          description: "Посмотрите самые ходовые легковые типоразмеры, готовые к отгрузке.",
          url: "#collections",
          icon: "card",
      ...
      header.innerHTML = `
        ...
        <button class="dng-close-btn" aria-label="Закрыть">×</button>
      `;
      ...
      function createCountdown(countdownDate) {
        ...
        label.textContent = "Превью заканчивается через";
        ...
        function updateCountdown() {
          ...
          if (distance < 0) {
            timer.innerHTML = '<span style="color: #35015F; font-weight: 600;">Превью завершено</span>';
            return;
          }
