...
(function () {
    ...
    const LANGUAGES = {
        ru: {
            modalTitle: 'События Dengage',
            modalSubtitle: 'Редактируйте payload и быстро отправляйте события устройства.',
            helpTitle: '// нативные события SDK -> стандартные таблицы Data Space',
            helpText: 'Первые шесть карточек вызывают нативные действия SDK (<em>pageView</em>, <em>ec:addToCart</em>, <em>ec:beginCheckout</em>, <em>ec:addToWishlist</em>, <em>ec:order</em>), которые пишут в стандартные таблицы со всеми колонками. <em>key</em>, <em>event_date</em> и <em>session_id</em> заполняет сам SDK. Последние две используют sendDeviceEvent для пользовательских таблиц.',
            addCustomBtn: 'Создать новое пользовательское событие',
            addFieldBtn: '+ Добавить поле',
            sendBtn: 'Отправить событие',
            ...
            keyPlaceholder: 'имя_поля',
            valuePlaceholder: 'значение',
            errorEmptyTable: 'Имя таблицы не может быть пустым.',
            errorEmptyKey: 'Имена полей не могут быть пустыми.',
            iconTitle: 'События',
            iconSubtitle: 'Панель управления',

            events: [
                { title: 'Просмотр товара', description: 'pageView со всеми колонками → таблица page_view_events.' },
                { title: 'Просмотр категории', description: 'pageView категории → таблица page_view_events.' },
                { title: 'Добавлено в корзину', description: 'ec:addToCart с cartItems → таблица shopping_cart_events.' },
                { title: 'Добавлено в избранное', description: 'ec:addToWishlist → таблица wishlist_events.' },
                { title: 'Начало оформления', description: 'ec:beginCheckout с cartItems → таблица shopping_cart_events.' },
                { title: 'Оплата завершена', description: 'ec:order с cartItems → таблицы order_events и order_events_detail.' },
                { title: 'Обращение к консультанту', description: 'sendDeviceEvent → пользовательская таблица events.' },
                { title: 'Расчёт полной загрузки', description: 'sendDeviceEvent → пользовательская таблица events.' }
            ]
        },
        en: {
    ...
    const EVENT_TEMPLATES = [
        {
            ...
            properties: [
                ...
                { key: 'category_id', value: 'GRZ-ELOCHKA' },
                { key: 'promotion_id', value: 'PREDLOZHENIE-NEDELI' },
                { key: 'price', value: '2090.00' },
                { key: 'discounted_price', value: '1890.00' },
                { key: 'stock_count', value: '12' },
                { key: 'category_path', value: 'Шины > Грузовые > Ёлочка' }
            ]
        },
        {
            ...
            properties: [
                ...
                { key: 'page_title', value: 'Грузовая линейка | CantuPneus' },
                { key: 'category_id', value: 'LINEYKA-GRUZOVYE' },
                { key: 'category_path', value: 'Шины > Грузовые' },
                { key: 'promotion_id', value: 'BESPLATNAYA-DOSTAVKA-5000' }
            ]
        },
        {
        ...
        },
        {
            ...
            properties: [
                ...
                { key: 'list_name', value: 'izbrannye-tiporazmery' }
            ]
        },
        {
        ...
        },
        {
            ...
            properties: [
                ...
                { key: 'shipping', value: 'dostavka_cif' },
        ...
        },
        {
            ...
            properties: [
                ...
                { key: 'store_location', value: 'Филиал Итажаи' },
    ...
    function createModal() {
        ...
        langBtnTr.className = 'lang-btn' + (currentLang === 'ru' ? ' active' : '');
        langBtnTr.textContent = '🇷🇺 RU';
        langBtnTr.onclick = () => switchLanguage('ru');
    ...
    function switchLanguage(lang) {
        ...
        // Update buttons
        elements.langBtnTr.classList.toggle('active', lang === 'ru');
