...
<div id="dengage-modal" role="document">\
  <header id="dengage-header">\
    <h1 class="dengage-title">CantuPneus Reveal: узнайте свой приз</h1>\
    <p class="dengage-description">Сотрите защитный слой, чтобы узнать промокод CantuPneus и получить скидку на следующий заказ.</p>\
  </header>\
  <main id="dengage-body">\
    <div id="dengage-prize-card">\
      <div id="dengage-prize-inner">\
        <div id="dengage-prize-label">Поздравляем! Вы получили:</div>\
        <div id="dengage-prize-code" aria-hidden="true">'+ DENGAGE_PRIZE_CODE +'</div>\
      </div>\
      <canvas id="dengage-scratch-canvas" aria-label="Область стирания"></canvas>\
    </div>\
    <div class="dengage-note">Проведите пальцем или мышью по золотому слою. Код появится, как только вы начнёте.</div>\
  </main>\
  <footer id="dengage-footer">\
    <button id="dengage-close-btn" class="dengage-btn dengage-btn-close" aria-label="Закрыть">Закрыть</button>\
    <button id="dengage-copy-btn" class="dengage-btn dengage-btn-copy" disabled>Скопировать код</button>\
  </footer>\
</div>\
</div>';
...
try {
  ...
  alert('Не удалось скопировать. Выделите и скопируйте код вручную: ' + text);
}
...
function WheelGame() {
	...
	if (!document.getElementById('dng-cjs-game-form')) {
		...
		var html = '<div id="dng-cjs-game-form-overlay"></div><div id="dng-cjs-game-form">\
			<button id="dng-cjs-form-close" style="position: absolute; width:auto; right: 0px; top: 0px; border: 0px; color: #fff; padding: 5px 10px; cursor: pointer; font-size: 29px; transition: all 1s ease 0s; border-radius: 10px; background-color: rgba(0, 0, 0, 0); z-index: 999; transform: translate3d(0px, 0px, 3px);">✖</button>\
    			...
    			<div id="dng-cjs-wheel-content">\
    				<div id="dng-cjs-text-bold" class="dng-cjs-text">Крутите колесо CantuPneus</div>\
    				<div id="dng-cjs-text-normal" class="dng-cjs-text">Крутите колесо и выигрывайте скидки и подарки CantuPneus.</div>\
    				<button id="spin" class="dng-cjs-spin-button">Крутить колесо</button>\
    			</div>\
        	</div>';
		...
		function finalize(coupon, text, link) {
			...
			dngWheelContent.insertAdjacentHTML('beforeend', '<div id="dng-cjs-result-title" class="dng-cjs-text">Поздравляем!</div>');
			dngWheelContent.insertAdjacentHTML('beforeend', '<div id="dng-cjs-result-text" class="dng-cjs-text">' + text + '</div>');
			dngWheelContent.insertAdjacentHTML('beforeend', '<div id="dng-cjs-result-label" class="dng-cjs-text">Используйте код в следующем заказе до 31 декабря.</div>');
			dngWheelContent.insertAdjacentHTML('beforeend', '<input type="text" value="' + coupon + '" id="dng-cjs-wheelCouponCode" readonly/>');
			dngWheelContent.insertAdjacentHTML('beforeend', '<div id="dng-cjs-wheelCopyButton">Скопировать код и открыть каталог</div>');
			dngWheelContent.insertAdjacentHTML('beforeend', '<div id="dng-cjs-label" class="dng-cjs-text">*Предложение не суммируется с другими акциями.</div>');
			dngWheelContent.insertAdjacentHTML('beforeend', '<div id="dng-cjs-label-2" class="dng-cjs-text">*Действует на отдельные типоразмеры CantuPneus.</div>');
			...
			function WheelGame() {
				var spinTheWheel = function () {
					wheel.spin(function (valueSpun) {
						...
						if (valueSpun == "points") {
							coupon = "CANTU1000PTS";
							text = "Вы выиграли 1 000 баллов CantuPneus, действуют на любой заказ.";
							link = "#collections";
						}
						else if (valueSpun == "freight") {
							coupon = "CANTUFRETE";
							text = "Вы выиграли бесплатную доставку CIF для следующего заказа полной загрузки.";
							link = "#contact";
						}
						else if (valueSpun == "voucher") {
							coupon = "CANTU50BRL";
							text = "Вы выиграли сертификат на R$ 250 для отдельных типоразмеров.";
							link = "#collections";
						}
						else if (valueSpun == "platinum") {
							coupon = "CANTUPLATINUM";
							text = "Вы вступили в программу Platinum Fleet с эксклюзивными условиями CantuPneus.";
							link = "#contact";
						}
						else if (valueSpun == "pricelist") {
							coupon = "CANTUNOVO";
							text = "Вы получили ранний доступ к следующему прайс-листу.";
							link = "#collections";
						}
						else if (valueSpun == "gift") {
							coupon = "CANTUCARGA";
							text = "Вы выиграли подарок-сюрприз CantuPneus к следующему заказу.";
							link = "#contact";
						}
						else {
							alert(valueSpun);
...
<div class="tinder--status">
  ...
  <div class="tinder--cards">
    <div data-id="carga" class="tinder--card">
     <img src="images/scenes/hero-carga.jpg" alt="Грузовая линейка">
     <div class="tinder--card-label">
       <span class="eyebrow">Грузовая линейка</span>
    ...
    <div data-id="heritage" class="tinder--card">
     <img src="images/scenes/linha-agricola.jpg" alt="Индустриальная линейка и OTR">
     <div class="tinder--card-label">
       <span class="eyebrow">Операции CantuPneus</span>
       <span class="title">Aeolus HN08 Neo 295/80 R22.5</span>
     </div>
    </div>
    <div data-id="statement" class="tinder--card">
     <img src="images/scenes/linha-otr.jpg" alt="Сельскохозяйственная линейка">
     <div class="tinder--card-label">
       <span class="eyebrow">Сельскохозяйственная линейка</span>
       <span class="title">CEAT Farmax R1W 18.4-34</span>
        ...
        <div class="tinder--buttons">
          ...
          <button id="love" aria-label="Сохранить"><svg class="fa fa-heart" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 21s-7.5-4.9-9.6-9A5.6 5.6 0 0 1 12 6.2 5.6 5.6 0 0 1 21.6 12c-2.1 4.1-9.6 9-9.6 9Z"/></svg></button>
        </div>
</div>`;
...
slideright++;
slideleft=0;
console.log("Сохранено:" , slideright);
}
...
code="CANTU-CARGA";
title="Marshal KLD01 сохранён в вашем списке CantuPneus";
subtitle="Сообщим, как только откроется новая партия грузовой линейки.";
break;
case "recap":
image="images/scenes/story-frota.jpg";
code="CANTU-KLD01";
title="Marshal KLD01 295/80 R22.5 добавлен в избранное";
subtitle="Получите прайс-лист раньше всех.";
break;
case "heritage":
image="images/scenes/linha-agricola.jpg";
code="CANTU-FROTA";
title="Aeolus AL37 OTR отмечен для вас";
subtitle="Консультант CantuPneus расскажет об условиях полной загрузки.";
break;
case "statement":
image="images/scenes/linha-otr.jpg";
code="CANTU-AGRO";
title="Kumho Solus TA31 сохранён";
subtitle="Смотрите другие типоразмеры на специальных условиях для автопарков.";
break;
default:
image="images/scenes/linha-carga.jpg";
code="CANTU-BRINDE";
title="Вас ждёт специальное предложение CantuPneus";
subtitle="Свяжитесь с консультантом CantuPneus, чтобы активировать выгоду.";
}
...
<button id="dng-cjs-form-close" class="dng-cjs-ChoosePopup-close" type="button" aria-label="Закрыть">✖</button>
