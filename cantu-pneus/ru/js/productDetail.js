(function () {
    ...
    function renderProduct(product) {
        ...
        const colorsHtml = product.colors && product.colors.length
            ? '<div class="product-detail-meta-row">'
              + '<span class="product-detail-meta-label">Конструкция</span>'
        ...
        const brandHtml = product.brand
            ? '<div class="product-detail-meta-row">'
              + '<span class="product-detail-meta-label">Бренд</span>'
        ...
        const availabilityHtml = '<div class="product-detail-meta-row">'
            + '<span class="product-detail-meta-label">Наличие</span>'
            + '<span class="product-detail-meta-value ' + (product.availability ? 'is-available' : 'is-unavailable') + '">'
            + (product.availability ? 'В наличии' : 'Распродано')
            + '</span>'
            + '</div>';
        ...
        container.innerHTML = ''
            ...
            + '        <button class="cart-qty-btn" type="button" data-product-qty-decrement aria-label="Уменьшить количество">−</button>'
            + '        <span data-product-qty>1</span>'
            + '        <button class="cart-qty-btn" type="button" data-product-qty-increment aria-label="Увеличить количество">+</button>'
            + '      </div>'
            + '      <button class="btn-primary product-detail-add-btn" type="button" data-product-add-to-cart'
            + (product.availability ? '' : ' disabled')
            + '>'
            + (product.availability ? 'В корзину' : 'Недоступно')
            + '      </button>'
            + '    </div>'
            + '    <a class="btn-text" href="index.html#products">← Назад ко всем товарам</a>'
            + '  </div>'
            + '</div>';
        ...
        if (addBtn) {
            addBtn.addEventListener('click', function () {
                ...
                window.CantuCart.addItem({
                    ...
                    image: product.image,
                    category: getCategoryParts(product.category).pop() || 'Шины'
                }, quantity);
            });
        }
    }
    ...
    function renderError(message) {
        ...
        container.innerHTML = ''
            ...
            + '  <a class="btn-primary" href="index.html#products">Вернуться в каталог</a>'
            + '</div>';
    }
    ...
    function init() {
        ...
        if (!productId) {
            renderError('Товар не выбран.');
            return;
        }

        if (!window.CantuCatalogData) {
            renderError('Каталог недоступен.');
            return;
        }
        ...
        window.CantuCatalogData.getProductById(productId)
            ...
            .catch(function (error) {
                ...
                renderError('Сейчас не удалось загрузить этот товар.');
            });
    }
