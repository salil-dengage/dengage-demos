...
/* ----------------------------------------------------------------------------
   ...
   This site is the RU CantuPneus site, so its eight Default Scenarios and its A/B
   test fire as ru_survey, ru_nps-popup, ru_ab-testing and so on.
...
/* ---------------------------------------------------------------- THE SWITCH
   ...
   ON. This site fires ru_ prefixed scenario events, and the ru_ campaigns in
   the panel serve them from cantu-pneus/panel-content/ru/.
...
function sendScenarioEvent(item, menu) {
  ...
  try {
    ...
    console.error('Не удалось отправить событие сценария ' + item.slug + ':', err);
  }
}
...
function cantuCatalog() {
  ...
  var menuData = [
    {
      ...
      icon: '📱',
      note: 'Собраны в панели Dengage (аккаунт BFSI)',
      mode: 'panel',
      items: [
        { label: 'Опрос',            widgetName: 'Survey',              slug: 'survey' },
        { label: 'NPS-попап',           widgetName: 'Nps Popup',           slug: 'nps-popup' },
        { label: 'Попап подписки',  widgetName: 'Subscripton Popup',   slug: 'subscripton-popup' },
        { label: 'Закреплённая полоса',          widgetName: 'Stickey Bar',         slug: 'stickey-bar' },
        { label: 'Попап с изображением',     widgetName: 'Image Popup',         slug: 'image-popup' },
        { label: 'Полоса с изображением',     widgetName: 'Image Bar',           slug: 'image-bar' },
        { label: 'Горизонтальный попап',    widgetName: 'Horizonal Popup',     slug: 'horizonal-popup' },
        { label: 'Попап с изображением и CTA', widgetName: 'CTA Image Popup',     slug: 'cta-image-popup' }
      ]
    },
    {
      ...
      icon: '🔀',
      note: 'Одна кампания, три дизайна, распределение 10/30/30/30',
      mode: 'panel',
      items: [
    ...
    },
    {
      ...
      icon: '🎯',
      note: 'Накладываются на контент, вёрстка не меняется',
      items: [
        ...
        { label: 'Боковая панель',      widgetName: 'Side Bar',         slug: 'side-bar',
          fn: function () {
            if (!window.EaringWidget) earingWidget();
            window.EaringWidget.open();
          } },
        { label: 'Нижний ассистент',widgetName: 'Bottom Assistant', slug: 'bottom-assistant',
          fn: function () {
            if (!window.openBottomAssistant) BottomAssistant();
            window.openBottomAssistant();
          } },
        { label: 'Карусельный баннер',   widgetName: 'Carousel Banner',  slug: 'carousel-banner',  fn: function () { carouselBanner(); } }
      ]
    },
    {
      ...
      icon: '🎮',
      note: 'Механики вовлечения с призами',
      items: [
        { label: 'Колесо фортуны', widgetName: 'Spin to Win',    slug: 'spin-to-win',    fn: function () { WheelGame(); } },
        { label: 'Скретч-карта',      widgetName: 'Scratch to Win', slug: 'scratch-to-win', fn: function () { ScratchGame(); } },
        ...
        { label: 'Снег',            widgetName: 'Snow',           slug: 'snow',           fn: function () { SnowStorm(); } }
      ]
    },
    {
      ...
      icon: '◆',
      note: 'На основе каталога шин',
      items: [
        { label: 'Классический виджет', widgetName: 'Classic Widget', slug: 'classic-widget', reveal: '#classicWidget', fn: function () { ClassicWidget(); } },
        { label: 'Баннерный виджет',   widgetName: 'Banner Widget',  slug: 'banner-widget',  reveal: '#bannerWidget', fn: function () { BannerWidget(); } },
        { label: 'Виджет с вкладками',  widgetName: 'Tab Widget',     slug: 'tab-widget',     reveal: '#tabWidget', fn: function () { TabWidget(); } },
        { label: 'Боковой виджет',  widgetName: 'SideBar Widget', slug: 'sidebar-widget', fn: function () { SideBarWidget(); } },
        { label: 'Попап-виджет',    widgetName: 'Popup Widget',   slug: 'popup-widget',   fn: function () { PopupWidget(); } }
      ]
    }
  ];
  ...
  stickyIcon.setAttribute('aria-label', 'Открыть каталог сценариев Dengage');
  ...
  modal.setAttribute('aria-label', 'Каталог Dengage');
  modal.style.cssText =
  ...
  title.textContent = 'Каталог Dengage';
  title.style.cssText =
  ...
  closeBtn.setAttribute('aria-label', 'Закрыть каталог');
  ...
  counter.textContent = total + ' сценариев On-Site. Каждая кнопка отправляет событие в dataLayer с указанным слагом.';
  ...
  function paintPush() {
    ...
    if (state === 'granted') {
      pushLabel.textContent = 'Push-уведомления включены';
      pushBtn.textContent = 'Проверить';
      pushBtn.disabled = false;
    } else if (state === 'denied') {
      pushLabel.textContent = 'Уведомления заблокированы в браузере';
      pushBtn.textContent = 'Заблокировано';
      pushBtn.disabled = true;
      pushBtn.style.opacity = '.5';
      pushBtn.style.cursor = 'not-allowed';
    } else {
      pushLabel.textContent = 'Push-уведомления';
  ...
  pushBtn.addEventListener('click', function (e) {
    e.stopPropagation();
    if (typeof window.dengage !== 'function') {
      console.warn('SDK Dengage ещё не загрузился.');
      return;
    }
    ...
    try {
      ...
      console.warn('showCustomPrompt недоступен:', err);
    }
    ...
    setTimeout(function () {
      ...
      console.log('Открываем браузерный запрос разрешения.');
      try {
  ...
  resetLabel.textContent = 'Повторять сценарии без очистки браузера';

  var resetBtn = document.createElement('button');
  resetBtn.style.cssText = pushBtn.style.cssText;
  resetBtn.textContent = 'Сбросить показы';
