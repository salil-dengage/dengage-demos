...
function cantuLoadProducts(widgetId, productsUrl, onReady, label) {
  ...
  try {
    ...
    console.error(label + ': getRecommendation не сработал, используем локальный каталог', err);
    local();
  }
}
            ...
            <div class="swiper-slide">
                <article class="reco-product-card">
                    ...
                    <div class="reco-product-body">
                        ...
                        >${product.availability ? 'В корзину' : 'Нет в наличии'}</button>
                    </div>
                </article>
            </div>
        `;
        ...
        widget.innerHTML = `
            <div class="container">
                <div class="reco-widget-header">
                    <div>
                        <p class="text-label">Подобрано для вас</p>
                        <h2 class="reco-widget-title">Лидер продаж</h2>
                        <p class="reco-widget-subtitle">Самые ходовые типоразмеры этого сезона</p>
                    </div>
                </div>
            ...
            <div class="container">
                <div class="banner-widget-header">
                    <div class="banner-widget-heading">
                        <p class="text-label">Выбор консультанта</p>
                        <h2 class="banner-widget-title">Рекомендуем вам</h2>
                        <p class="banner-widget-subtitle">Шины, подобранные под ваш профиль эксплуатации</p>
                    </div>
                ...
                <div class="banner-widget-body">
                    <aside class="banner-widget-banner">
                        ...
                        <div class="banner-widget-banner-overlay">
                            <p class="banner-widget-banner-label">Выбор эксперта</p>
                            <h3 class="banner-widget-banner-title">${escapeHtml(bannerProduct.name)}</h3>
                            <a href="#collections" class="banner-widget-banner-link">Смотреть всю линейку</a>
                        </div>
                    </aside>
...
function TabWidget() {
    ...
    /* match is tested against the feed's category path, for example
       "Шины > Грузовые > Ёлочка". Keep these in step with
       cantu_prod_example.json or a tab renders empty. */
    const CATEGORIES = [
        { id: 'carga', label: 'Грузовые', match: '> Грузовые >' },
        { id: 'passeio', label: 'Легковые', match: '> Легковые >' },
        { id: 'agricola', label: 'Сельскохозяйственные', match: '> Сельскохозяйственные >' },
        { id: 'industrial', label: 'Индустриальные и OTR', match: '> Индустриальные и OTR >' }
    ];
            ...
            <div class="container">
                <div class="tab-widget-header">
                    <div class="tab-widget-heading">
                        ...
                        <h2 class="tab-widget-title">Лидеры продаж по категориям</h2>
                        <p class="tab-widget-subtitle">Самые ходовые типоразмеры в наиболее востребованных линейках</p>
                    </div>
                </div>
...
function SideBarWidget() {
    ...
    function render(products) {
        ...
        widget.innerHTML = `
            <button class="mv-tab" type="button" aria-label="Открыть и закрыть «Часто просматриваемые»">
                <span class="mv-tab-icon">›</span>
                <span>Часто просматриваемые</span>
            </button>
            <div class="mv-panel">
                <p class="mv-panel-subtitle">Trending</p>
                <h3 class="mv-panel-title">Часто просматриваемые</h3>
                <div class="mv-list">
        ...
        overlay.innerHTML = `
            <div id="${WIDGET_ID}" role="dialog" aria-modal="true" aria-labelledby="${WIDGET_ID}-title">
                ...
                <div class="popup-header">
                    <p class="popup-label">Персонально для вас</p>
                    <h2 class="popup-title" id="${WIDGET_ID}-title">Рекомендуем вам</h2>
                    <p class="popup-subtitle">Типоразмеры, подобранные по вашей истории покупок</p>
                </div>
