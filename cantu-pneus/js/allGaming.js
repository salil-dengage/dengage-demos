...
<div id="dengage-modal" role="document">\
  <header id="dengage-header">\
    <h1 class="dengage-title">CantuPneus Reveal: descubra seu prêmio</h1>\
    <p class="dengage-description">Raspe para revelar seu código promocional CantuPneus e liberar o desconto no próximo pedido.</p>\
  </header>\
  <main id="dengage-body">\
    <div id="dengage-prize-card">\
      <div id="dengage-prize-inner">\
        <div id="dengage-prize-label">Parabéns! Você desbloqueou:</div>\
        <div id="dengage-prize-code" aria-hidden="true">'+ DENGAGE_PRIZE_CODE +'</div>\
      </div>\
      <canvas id="dengage-scratch-canvas" aria-label="Área de raspagem"></canvas>\
    </div>\
    <div class="dengage-note">Use o dedo ou o mouse para raspar a camada dourada. Seu código aparece assim que você começar.</div>\
  </main>\
  <footer id="dengage-footer">\
    <button id="dengage-close-btn" class="dengage-btn dengage-btn-close" aria-label="Fechar">Fechar</button>\
    <button id="dengage-copy-btn" class="dengage-btn dengage-btn-copy" disabled>Copiar código</button>\
  </footer>\
</div>\
</div>';
...
try {
  ...
  alert('Não foi possível copiar. Selecione e copie o código manualmente: ' + text);
}
...
function WheelGame() {
	...
	if (!document.getElementById('dng-cjs-game-form')) {
		...
		var html = '<div id="dng-cjs-game-form-overlay"></div><div id="dng-cjs-game-form">\
			<button id="dng-cjs-form-close" style="position: absolute; width:auto; right: 0px; top: 0px; border: 0px; color: #fff; padding: 5px 10px; cursor: pointer; font-size: 29px; transition: all 1s ease 0s; border-radius: 10px; background-color: rgba(0, 0, 0, 0); z-index: 999; transform: translate3d(0px, 0px, 3px);">✖</button>\
    			...
    			<div id="dng-cjs-wheel-content">\
    				<div id="dng-cjs-text-bold" class="dng-cjs-text">Gire a roleta CantuPneus</div>\
    				<div id="dng-cjs-text-normal" class="dng-cjs-text">Gire e concorra a descontos e brindes CantuPneus.</div>\
    				<button id="spin" class="dng-cjs-spin-button">Girar agora</button>\
    			</div>\
        	</div>';
		...
		function finalize(coupon, text, link) {
			...
			dngWheelContent.insertAdjacentHTML('beforeend', '<div id="dng-cjs-result-title" class="dng-cjs-text">Parabéns!</div>');
			dngWheelContent.insertAdjacentHTML('beforeend', '<div id="dng-cjs-result-text" class="dng-cjs-text">' + text + '</div>');
			dngWheelContent.insertAdjacentHTML('beforeend', '<div id="dng-cjs-result-label" class="dng-cjs-text">Use o código no próximo pedido até 31 de dezembro.</div>');
			dngWheelContent.insertAdjacentHTML('beforeend', '<input type="text" value="' + coupon + '" id="dng-cjs-wheelCouponCode" readonly/>');
			dngWheelContent.insertAdjacentHTML('beforeend', '<div id="dng-cjs-wheelCopyButton">Copiar código e ver ofertas</div>');
			dngWheelContent.insertAdjacentHTML('beforeend', '<div id="dng-cjs-label" class="dng-cjs-text">*Esta oferta não é cumulativa com outras promoções.</div>');
			dngWheelContent.insertAdjacentHTML('beforeend', '<div id="dng-cjs-label-2" class="dng-cjs-text">*Válido em medidas CantuPneus selecionadas.</div>');
			...
			function WheelGame() {
				var spinTheWheel = function () {
					wheel.spin(function (valueSpun) {
						...
						if (valueSpun == "points") {
							coupon = "CANTU1000PTS";
							text = "Você ganhou 1000 pontos CantuPneus, válidos em qualquer pedido.";
							link = "#collections";
						}
						else if (valueSpun == "freight") {
							coupon = "CANTUFRETE";
							text = "Você ganhou frete CIF grátis no próximo pedido de carga fechada.";
							link = "#contact";
						}
						else if (valueSpun == "voucher") {
							coupon = "CANTU50BRL";
							text = "Você ganhou um vale de R$ 250 válido em medidas selecionadas.";
							link = "#collections";
						}
						else if (valueSpun == "platinum") {
							coupon = "CANTUPLATINUM";
							text = "Você entrou no programa Frota Platinum, com condições exclusivas CantuPneus.";
							link = "#contact";
						}
						else if (valueSpun == "pricelist") {
							coupon = "CANTUNOVO";
							text = "Você garantiu acesso antecipado à próxima tabela de ofertas.";
							link = "#collections";
						}
						else if (valueSpun == "gift") {
							coupon = "CANTUCARGA";
							text = "Você ganhou um brinde surpresa CantuPneus no próximo pedido.";
							link = "#contact";
						}
						else {
							alert(valueSpun);
...
<div class="tinder--status">
  ...
  <div class="tinder--cards">
    <div data-id="carga" class="tinder--card">
     <img src="images/scenes/hero-carga.jpg" alt="Linha Carga">
     <div class="tinder--card-label">
       <span class="eyebrow">Linha Carga</span>
    ...
    <div data-id="heritage" class="tinder--card">
     <img src="images/scenes/linha-agricola.jpg" alt="Linha Industrial e OTR">
     <div class="tinder--card-label">
       <span class="eyebrow">Operação CantuPneus</span>
       <span class="title">Aeolus HN08 Neo 295/80 R22.5</span>
     </div>
    </div>
    <div data-id="statement" class="tinder--card">
     <img src="images/scenes/linha-otr.jpg" alt="Linha Agrícola">
     <div class="tinder--card-label">
       <span class="eyebrow">Linha Agrícola</span>
       <span class="title">CEAT Farmax R1W 18.4-34</span>
        ...
        <div class="tinder--buttons">
          ...
          <button id="love" aria-label="Salvar"><svg class="fa fa-heart" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M12 21s-7.5-4.9-9.6-9A5.6 5.6 0 0 1 12 6.2 5.6 5.6 0 0 1 21.6 12c-2.1 4.1-9.6 9-9.6 9Z"/></svg></button>
        </div>
</div>`;
...
code="CANTU-CARGA";
title="Marshal KLD01 salvo na sua lista CantuPneus";
subtitle="Avisamos você assim que abrir um novo lote da linha de carga.";
break;
case "recap":
image="images/scenes/story-frota.jpg";
code="CANTU-KLD01";
title="Marshal KLD01 295/80 R22.5 salvo nos favoritos";
subtitle="Receba a tabela de ofertas antes de todo mundo.";
break;
case "heritage":
image="images/scenes/linha-agricola.jpg";
code="CANTU-FROTA";
title="Aeolus AL37 OTR anotado para você";
subtitle="Um consultor CantuPneus vai apresentar as condições de carga fechada.";
break;
case "statement":
image="images/scenes/linha-otr.jpg";
code="CANTU-AGRO";
title="Kumho Solus TA31 salvo";
subtitle="Veja mais medidas com condição especial para frota.";
break;
default:
image="images/scenes/linha-carga.jpg";
code="CANTU-BRINDE";
title="Uma condição especial CantuPneus espera por você";
subtitle="Fale com seu consultor CantuPneus para liberar o benefício.";
}
...
<button id="dng-cjs-form-close" class="dng-cjs-ChoosePopup-close" type="button" aria-label="Fechar">✖</button>
