            ...
            cartItemsEl.innerHTML = summary.items.map(item => `
                <article class="cart-item" data-cart-item-id="${item.id}">
                    ...
                    <div>
                        ...
                        <p class="cart-item-meta">${escapeHtml(item.category || 'Pneus')}</p>
                        <p class="cart-item-price">${formatMoney(item.price, item.currency)} × ${item.quantity}</p>
                        <div class="cart-item-actions">
                            <button class="cart-qty-btn" type="button" data-cart-decrement="${escapeHtml(item.id)}" aria-label="Diminuir quantidade">−</button>
                            <span>${item.quantity}</span>
                            <button class="cart-qty-btn" type="button" data-cart-increment="${escapeHtml(item.id)}" aria-label="Aumentar quantidade">+</button>
                            <button class="cart-remove-btn" type="button" data-cart-remove="${escapeHtml(item.id)}">Remover</button>
                        </div>
                    </div>
                </article>
            `).join('');
