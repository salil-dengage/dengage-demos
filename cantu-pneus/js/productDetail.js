(function () {
    ...
    function renderProduct(product) {
        ...
        const colorsHtml = product.colors && product.colors.length
            ? '<div class="product-detail-meta-row">'
              + '<span class="product-detail-meta-label">Construçãos</span>'
        ...
        const brandHtml = product.brand
            ? '<div class="product-detail-meta-row">'
              + '<span class="product-detail-meta-label">Marca</span>'
        ...
        const availabilityHtml = '<div class="product-detail-meta-row">'
            + '<span class="product-detail-meta-label">Disponibilidade</span>'
            + '<span class="product-detail-meta-value ' + (product.availability ? 'is-available' : 'is-unavailable') + '">'
            + (product.availability ? 'Em estoque' : 'Esgotado')
            + '</span>'
            + '</div>';
        ...
        container.innerHTML = ''
            ...
            + '        <button class="cart-qty-btn" type="button" data-product-qty-decrement aria-label="Diminuir quantidade">−</button>'
            + '        <span data-product-qty>1</span>'
            + '        <button class="cart-qty-btn" type="button" data-product-qty-increment aria-label="Aumentar quantidade">+</button>'
            + '      </div>'
            + '      <button class="btn-primary product-detail-add-btn" type="button" data-product-add-to-cart'
            + (product.availability ? '' : ' disabled')
            + '>'
            + (product.availability ? 'Adicionar ao carrinho' : 'Indisponível')
            + '      </button>'
            + '    </div>'
            + '    <a class="btn-text" href="index.html#products">← Voltar para todos os produtos</a>'
            + '  </div>'
            + '</div>';
        ...
        if (addBtn) {
            addBtn.addEventListener('click', function () {
                ...
                window.CantuCart.addItem({
                    ...
                    image: product.image,
                    category: getCategoryParts(product.category).pop() || 'Pneus'
                }, quantity);
            });
        }
    }
    ...
    function renderError(message) {
        ...
        container.innerHTML = ''
            ...
            + '  <a class="btn-primary" href="index.html#products">Voltar ao catálogo</a>'
            + '</div>';
    }
    ...
    function init() {
        ...
        if (!productId) {
            renderError('Nenhum produto selecionado.');
            return;
        }

        if (!window.CantuCatalogData) {
            renderError('O catálogo não está disponível.');
            return;
        }
        ...
        window.CantuCatalogData.getProductById(productId)
            ...
            .catch(function (error) {
                ...
                renderError('Não foi possível carregar este produto agora.');
            });
    }
