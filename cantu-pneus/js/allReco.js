...
function cantuLoadProducts(widgetId, productsUrl, onReady, label) {
  ...
  try {
    ...
    console.error(label + ': getRecommendation falhou, usando catálogo local', err);
    local();
  }
}
            ...
            <div class="swiper-slide">
                <article class="reco-product-card">
                    ...
                    <div class="reco-product-body">
                        ...
                        >${product.availability ? 'Adicionar ao carrinho' : 'Sem estoque'}</button>
                    </div>
                </article>
            </div>
        `;
        ...
        widget.innerHTML = `
            <div class="container">
                <div class="reco-widget-header">
                    <div>
                        <p class="text-label">Selecionado para você</p>
                        <h2 class="reco-widget-title">Mais vendido</h2>
                        <p class="reco-widget-subtitle">As medidas com maior giro nesta temporada</p>
                    </div>
                </div>
            ...
            <div class="container">
                <div class="banner-widget-header">
                    <div class="banner-widget-heading">
                        <p class="text-label">Seleção do consultor</p>
                        <h2 class="banner-widget-title">Recomendado para você</h2>
                        <p class="banner-widget-subtitle">Pneus recomendados para o seu perfil de operação</p>
                    </div>
                ...
                <div class="banner-widget-body">
                    <aside class="banner-widget-banner">
                        ...
                        <div class="banner-widget-banner-overlay">
                            <p class="banner-widget-banner-label">Escolha do especialista</p>
                            <h3 class="banner-widget-banner-title">${escapeHtml(bannerProduct.name)}</h3>
                            <a href="#collections" class="banner-widget-banner-link">Ver linha completa</a>
                        </div>
                    </aside>
...
function TabWidget() {
    ...
    /* match is tested against the feed's category path, for example
       "Pneus > Carga > Borrachudo". Keep these in step with
       cantu_prod_example.json or a tab renders empty. */
    const CATEGORIES = [
        { id: 'carga', label: 'Carga', match: '> Carga >' },
        { id: 'passeio', label: 'Passeio', match: '> Passeio >' },
        { id: 'agricola', label: 'Agrícola', match: '> Agrícola >' },
        { id: 'industrial', label: 'Industrial e OTR', match: '> Industrial e OTR >' }
    ];
            ...
            <div class="container">
                <div class="tab-widget-header">
                    <div class="tab-widget-heading">
                        ...
                        <h2 class="tab-widget-title">Mais vendidos da categoria</h2>
                        <p class="tab-widget-subtitle">As medidas com maior giro nas linhas mais procuradas</p>
                    </div>
                </div>
...
function SideBarWidget() {
    ...
    function render(products) {
        ...
        widget.innerHTML = `
            <button class="mv-tab" type="button" aria-label="Abrir e fechar Mais vistos">
                <span class="mv-tab-icon">›</span>
                <span>Mais vistos</span>
            </button>
            <div class="mv-panel">
                <p class="mv-panel-subtitle">Trending</p>
                <h3 class="mv-panel-title">Mais vistos</h3>
                <div class="mv-list">
        ...
        overlay.innerHTML = `
            <div id="${WIDGET_ID}" role="dialog" aria-modal="true" aria-labelledby="${WIDGET_ID}-title">
                ...
                <div class="popup-header">
                    <p class="popup-label">Personalizado para você</p>
                    <h2 class="popup-title" id="${WIDGET_ID}-title">Recomendado para você</h2>
                    <p class="popup-subtitle">Medidas selecionadas a partir do seu histórico de compra</p>
                </div>
